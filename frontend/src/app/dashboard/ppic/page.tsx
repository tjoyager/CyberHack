...
export default function PPICManagerPage() {
  ...
  const [assignmentModal, setAssignmentModal] = useState<{ open: boolean; lot: any | null }>({
    open: false,
    lot: null,
  });
  ...
  const fetchData = async () => {
    try {
      ...
      // Only show APPROVED and IN_PRODUCTION for PPIC Manager
      const relevantLots = lotsData.filter((l: any) => 
        l.status === "APPROVED" || l.status === "IN_PRODUCTION"
      );
          ...
          <div className="bg-white rounded-2xl border border-border shadow-2xl w-full max-w-md overflow-hidden">
            ...
            <div className="p-6 border-t border-border flex flex-col gap-3 bg-slate-50/50">
              <button
                ...
                disabled={!assignmentData.warehouseSlot || submitting}
                className="flex items-center justify-center gap-2 w-full px-4 py-2.5 rounded-lg bg-primary text-white hover:bg-primary/90 transition-colors font-bold text-sm shadow-md shadow-primary/20 disabled:opacity-50 disabled:cursor-not-allowed"
              >
              ...
              <button
                onClick={() => setAssignmentModal({ open: false, lot: null })}
                className="w-full px-4 py-2 rounded-lg border border-border bg-white hover:bg-slate-50 transition-colors text-foreground font-semibold text-sm"
              >
                Cancel
              </button>
            </div>
