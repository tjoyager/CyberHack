...
export default function QCInspectorPage() {
  ...
  const [inspectionModal, setInspectionModal] = useState<{ open: boolean; lot: any | null }>({ open: false, lot: null });
  const [inspectionData, setInspectionData] = useState({ qc_notes: "", rejection_reason: "" });
  ...
  if (loading) {
    return <div className="flex items-center justify-center h-64"><LoaderIcon className="w-8 h-8 animate-spin text-primary" /></div>;
  }
                ...
                <tr key={lot.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-center">
                    {lot.status === "PENDING_QC" ? (
                      <button onClick={() => openInspection(lot)} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-primary text-white hover:bg-primary/90 transition-all font-bold text-xs shadow-sm shadow-primary/20">
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-6 z-50 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl border border-border shadow-2xl w-full max-w-md overflow-hidden">
            <div className="p-6 border-b border-border flex items-center justify-between bg-slate-50/50">
              ...
              <button onClick={() => setInspectionModal({ open: false, lot: null })} className="w-8 h-8 rounded-lg hover:bg-white transition-colors flex items-center justify-center text-muted-foreground border border-transparent hover:border-border">
            ...
            <div className="p-6 border-t border-border flex gap-3 bg-slate-50/50">
              <button onClick={() => handleUpdateStatus('APPROVED')} disabled={submitting} className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-green-600 text-white hover:bg-green-700 transition-colors font-bold text-sm shadow-md shadow-green-200 disabled:opacity-50">
                {submitting ? <LoaderIcon className="w-5 h-5 animate-spin" /> : <CheckIcon className="w-5 h-5" />}Approve Lot
              </button>
              <button onClick={() => handleUpdateStatus('REJECTED')} disabled={submitting || !inspectionData.rejection_reason} className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-red-600 text-white hover:bg-red-700 transition-colors font-bold text-sm shadow-md shadow-red-200 disabled:opacity-50">
                {submitting ? <LoaderIcon className="w-5 h-5 animate-spin" /> : <XIcon className="w-5 h-5" />}Reject Lot
              </button>
            </div>
            <div className="p-6 pt-0 flex justify-center bg-slate-50/50">
              <button onClick={() => setInspectionModal({ open: false, lot: null })} className="px-4 py-2 rounded-lg border border-border bg-white hover:bg-slate-50 transition-colors text-foreground font-semibold text-sm">
                Cancel
              </button>
            </div>
          </div>
