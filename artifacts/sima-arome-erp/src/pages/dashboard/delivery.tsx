...
export default function DeliveryStaffPage() {
  ...
  const [deliveryModal, setDeliveryModal] = useState<{ open: boolean; lot: any | null }>({ open: false, lot: null });
  const [deliveryData, setDeliveryData] = useState({ driver_name: "", vehicle_plate: "", destination: "" });
  ...
  const fetchData = async () => {
    try {
      ...
      const relevantLots = lotsData.filter((l: any) => l.status === "IN_PRODUCTION" || l.status === "DELIVERED");
                ...
                <tr key={lot.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-center">
                    {lot.status === "IN_PRODUCTION" ? (
                      <button onClick={() => openDelivery(lot)} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-primary text-white hover:bg-primary/90 transition-all font-bold text-xs shadow-sm shadow-primary/20">
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-6 z-[100] animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl border border-border shadow-2xl w-full max-w-md animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-border flex items-center justify-between">
              ...
              <button onClick={() => setDeliveryModal({ open: false, lot: null })} className="w-8 h-8 rounded-lg hover:bg-background transition-colors flex items-center justify-center text-muted-foreground">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-6 space-y-4">
              <div>
                ...
                <input type="text" value={deliveryData.destination} onChange={(e) => setDeliveryData({ ...deliveryData, destination: e.target.value })} className="w-full px-4 py-2 border border-border rounded-lg bg-input-background focus:outline-none focus:ring-2 focus:ring-ring text-sm font-medium" placeholder="e.g., Main Distribution Center" />
              </div>
              <div>
                <label className="block mb-2 text-xs font-bold text-muted-foreground uppercase tracking-wider">Driver Name</label>
                <input type="text" value={deliveryData.driver_name} onChange={(e) => setDeliveryData({ ...deliveryData, driver_name: e.target.value })} className="w-full px-4 py-2 border border-border rounded-lg bg-input-background focus:outline-none focus:ring-2 focus:ring-ring text-sm font-medium" placeholder="e.g., John Doe" />
              </div>
              <div>
                <label className="block mb-2 text-xs font-bold text-muted-foreground uppercase tracking-wider">Vehicle Plate</label>
                <input type="text" value={deliveryData.vehicle_plate} onChange={(e) => setDeliveryData({ ...deliveryData, vehicle_plate: e.target.value })} className="w-full px-4 py-2 border border-border rounded-lg bg-input-background focus:outline-none focus:ring-2 focus:ring-ring text-sm font-medium" placeholder="e.g., B 1234 XYZ" />
              </div>
            </div>
            <div className="p-6 border-t border-border flex flex-col gap-3">
              <button onClick={handleDeliver} disabled={!deliveryData.destination || submitting} className="flex items-center justify-center gap-2 w-full px-4 py-2.5 rounded-lg bg-primary text-white hover:bg-primary/90 transition-colors font-bold text-sm shadow-sm shadow-primary/20 disabled:opacity-50 disabled:cursor-not-allowed">
                {submitting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Truck className="w-4 h-4" />}Dispatch Delivery
              </button>
              <button onClick={() => setDeliveryModal({ open: false, lot: null })} className="w-full px-4 py-2.5 rounded-lg border border-border bg-background hover:bg-secondary/50 transition-colors text-foreground font-bold text-sm">
                Cancel
              </button>
            </div>
          </div>
