              ...
              {auditLogs.length === 0 ? (
                ...
                auditLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4 text-sm font-medium text-slate-600 whitespace-nowrap">{formatDate(log.timestamp)}</td>
                    <td className="px-6 py-4 text-sm font-bold text-foreground">{log.user?.username || "System"}</td>
                    <td className="px-6 py-4 text-sm text-foreground">
                      <span className="px-2 py-0.5 rounded bg-slate-100 text-slate-700 text-[10px] font-bold uppercase tracking-tight">{log.entity_name}</span>
                    </td>
