...
export default function IntakeStaffPage() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const matchedMaterial = materials.find(m => m.name.toLowerCase().includes(data.material_name?.toLowerCase()));
      const matchedSupplier = suppliers.find(s => s.company_name.toLowerCase().includes(data.supplier_name?.toLowerCase()));
      setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ material_id: "", supplier_id: "", quantity_kg: "" });
    } catch (error: any) {
    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="bg-white rounded-xl border border-border p-4 sm:p-6 shadow-sm">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 sm:mb-6">
          ...
          >
            {isAiLoading ? (
              <><Loader2 className="w-4 sm:w-5 h-4 sm:h-5 animate-spin" />Scanning...</>
            ) : (
              <><Sparkles className="w-4 sm:w-5 h-4 sm:h-5" />Auto-Fill with AI</>
            )}
          </button>
        </div>
        <form onSubmit={handleSubmit}>
          ...
          >
            {submitting ? <><Loader2 className="w-5 h-5 animate-spin" />Submitting...</> : <><Plus className="w-5 h-5" />Submit Lot</>}
          </button>
        </form>
      </div>
